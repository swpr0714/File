...
def intersect(origin, dir, obj):    # 射線與物體的相交測試
    if obj['type'] == 'plane':
        return intersect_plane(origin, dir, obj['position'], obj['normal'])
    elif obj['type'] == 'sphere':
        return intersect_sphere(origin, dir, obj['position'], obj['radius'])

def intersect_plane(origin, dir, point, normal):    # 射線與平面的相交測試
    dn = np.dot(dir, normal)
    if np.abs(dn) < 1e-6:   # 射線與平面幾乎平行
        return np.inf       # 交點為無窮遠處
    d = np.dot(point - origin, normal) / dn         # 交點與射線原點的距離（相似三角形原理）
    return d if d>0 else np.inf     # 負數表示射線射向平面的反方向

def intersect_sphere(origin, dir, center, radius):  # 射線與球的相交測試
    OC = center - origin
...
def get_normal(obj, point):         # 獲得物體表面某點處的單位法向量
    if obj['type'] == 'sphere':
...
scene = [sphere([.75, .1, 1.], .6, [.8, .3, 0.]),           # 球心位置，半徑，顏色
         sphere([-.3, .01, .2], .3, [.0, .0, .9]),
         sphere([-2.75, .1, 3.5], .6, [.1, .572, .184]),
         plane([0., -.5, 0.], [0., 1., 0.])]                # 平面上一點的位置，法向量
light_point = np.array([5., 5., -10.])                      # 點光源位置
light_color = np.array([1., 1., 1.])                        # 點光源的顏色值
ambient = 0.05                                              # 環境光
...
def intersect_color(origin, dir, intensity):
    ...
    for i, obj in enumerate(scene):
        ...
        if current_distance < min_distance:
            min_distance, obj_index = current_distance, i   # 記錄最近的交點距離和對應的物體
    if (min_distance == np.inf) or (intensity < 0.01):
        return np.array([0., 0., 0.])

    obj = scene[obj_index]
    P = origin + dir * min_distance     # 交點座標
    color = get_color(obj, P)
    N = get_normal(obj, P)                  # 交點處單位法向量
    ...
    l = [intersect(P + N * .0001, PL, obj_shadow_test)
            for i, obj_shadow_test in enumerate(scene) if i != obj_index]       # 陰影測試
    ...
    reflect_ray = dir - 2 * np.dot(dir, N) * N  # 計算反射光線
    c += obj['reflection'] * intersect_color(P + N * .0001, reflect_ray, obj['reflection'] * intensity)

    return np.clip(c, 0, 1)

w, h = 400, 300     # 螢幕寬高
O = np.array([0., 0.35, -1.])   # 攝像機位置
Q = np.array([0., 0., 0.])      # 攝像機指向
img = np.zeros((h, w, 3))
r = float(w) / h
S = (-1., -1. / r + .25, 1., 1. / r + .25)
